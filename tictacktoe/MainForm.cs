    ...
    public partial class MainForm : Form
    {
        ...
        void checkwin(int l, int m, int n)
        {
            ...
            if (cnt > 4)
            {
                ...
                {
                    if (cnt == 1)
                        declare(pl1 + " (Игрок X) Победил!");
                    if (cnt == 0)
                        declare("Победила дружба!");
                    reset();
                    if (vs == 1)
                        if (pl1 == "Компьютер")
                        {
        ...
        {
            if (MessageBox.Show(stmt + " Хотите сыграть еще раз?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
            {
                Application.Exit();
            }
        }
